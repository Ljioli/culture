...
// 根据时间戳计算耗时
function calDuration(startTimestamp, endTimestamp) {
	const duration = endTimestamp - startTimestamp; // 计算时间差（毫秒）
	const seconds = Math.floor(duration / 1000 % 60); // 计算秒
	const minutes = Math.floor(duration / (1000 * 60) % 60); // 计算分钟
	const hours = Math.floor(duration / (1000 * 60 * 60)); // 计算小时
...
function calDurationD(duration) { 
	const seconds = Math.floor(duration / 1000 % 60); // 计算秒
	const minutes = Math.floor(duration / (1000 * 60) % 60); // 计算分钟
	const hours = Math.floor(duration / (1000 * 60 * 60)); // 计算小时
