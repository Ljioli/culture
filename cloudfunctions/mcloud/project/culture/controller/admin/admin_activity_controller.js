...
class AdminActivityController extends BaseProjectAdminController {
  ...
  async getAdminActivityList() {
    ...
    for (let k = 0; k < list.length; k++) {
      list[k].ACTIVITY_ADD_TIME = timeUtil.timestamp2Time(
        list[k].ACTIVITY_ADD_TIME,
        "Y-M-D h:m:s",
      );

      list[k].statusDesc = service.getJoinStatusDesc(list[k]);

      list[k].ACTIVITY_START = timeUtil.timestamp2Time(
        list[k].ACTIVITY_START,
        "Y-M-D h:m",
      );
      list[k].ACTIVITY_END = timeUtil.timestamp2Time(
        list[k].ACTIVITY_END,
        "Y-M-D h:m",
      );
      list[k].ACTIVITY_STOP = timeUtil.timestamp2Time(
        list[k].ACTIVITY_STOP,
        "Y-M-D h:m",
      );
  ...
  async insertActivity() {
    ...
    let rules = {
      formTitle: "must|string|min:2|max:50|name=标题",
      cateId: "must|id|name=分类",
      cateName: "string|max:50|name=分类名称",
      formStart: "string|name=活动开始时间",
      formEnd: "string|name=活动结束时间",
      formStop: "must|string|name=报名截止时间",
      formAddress: "string|max:200|name=详细地址",
      formAddressGeo: "object|name=addressGeo",
      formMaxCnt: "int|min:0|max:9999|name=人数上限",
      formCancelSet: "int|in:0,1,2|name=取消设置",
      formCheckSet: "int|in:0,1|name=审核设置",
      formIsMenu: "int|in:0,1|name=公开展示名单",
      forms: "array|name=自定义表单",
      joinForms: "array|name=报名表单",
      formOrder: "int|min:0|max:9999|name=排序",
    };
    ...
    this.logOther("添加了活动《" + input.formTitle + "》");
  ...
  async getActivityDetail() {
    ...
    if (activity) {
      activity.ACTIVITY_START = timeUtil.timestamp2Time(
        activity.ACTIVITY_START,
        "Y-M-D h:m",
      );
      activity.ACTIVITY_END = timeUtil.timestamp2Time(
        activity.ACTIVITY_END,
        "Y-M-D h:m",
      );
      activity.ACTIVITY_STOP = timeUtil.timestamp2Time(
  ...
  async editActivity() {
    ...
    this.logOther("修改了活动《" + input.formTitle + "》");
  ...
  async delActivity() {
    ...
    if (title) this.logOther("删除了活动《" + title + "》");
  }
  ...
  async getActivityJoinList() {
    ...
    let list = result.list;
    for (let k = 0; k < list.length; k++) {
      list[k].ACTIVITY_JOIN_ADD_TIME = timeUtil.timestamp2Time(
        list[k].ACTIVITY_JOIN_ADD_TIME,
      );
    }
    result.list = list;
  ...
  async statusActivityJoin() {
    ...
    let service = new AdminActivityService();
    return await service.statusActivityJoin(
      input.activityJoinId,
      input.status,
      input.reason,
    );
  }
  ...
  async activityJoinDataGet() {
    ...
    if (input.isDel === 1) await service.deleteActivityJoinDataExcel(); //先删除
